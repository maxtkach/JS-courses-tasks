...
export class Lector extends User {
  ...
  renderCourses() {
    document.write(`${this.courses.map(course=>`<p class="user__courses--course student">Title: ${course.title} <span class="satisfactory">Lectors score:${course.score}</span> <span class="satisfactory">Average Students score:${course.studentsScore}</span></p>`).join(' ')})`);
  }
}